    ...
    <select id="priceType" class="form-select">
        ...
        <option value="wholesale">Wholesale Price</option>
            ...
            <div class="card">
                ...
                <div class="card-footer" id="cartFooter" style="display: none;">
                    <div class="row">
                        <div class="col-md-6">
                            <button id="clearCartBtn" class="btn btn-outline-danger btn-sm" type="button">
        ...
        <div class="col-md-5">
            <div class="card">
                ...
                <div class="card-body">
                <form id="posForm" action="{{ route('cashier.pos.store') }}" method="POST">
                        @csrf
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function showAlert(message, type = 'danger') {
        ...
        const existingAlerts = document.querySelectorAll('#alert-placeholder .alert:not([role="alert"])');
        ...
        cart.forEach((item, index) => {
            ...
            row.innerHTML = `
                ...
                <td class="text-center">
                    <button class="btn btn-sm btn-outline-danger remove-item" data-index="${index}" title="Remove" type="button">
            ...
            itemsInputs.innerHTML += `
                ...
                <input type="hidden" name="items[${index}][subtotal]" value="${lineTotal}">
            `;
        });
    ...
    function updateBalance() {
        ...
        // Color coding - FIXED: Customer owes if negative, change to give if positive
        if (balance < 0) {
            balanceEl.className = 'form-control bg-light text-danger';
            balanceEl.title = `Customer still owes: ${formatMoney(Math.abs(balance))}`;
        } else if (balance > 0) {
            balanceEl.className = 'form-control bg-light text-warning';
            balanceEl.title = `Change to give: ${formatMoney(balance)}`;
        } else {
...
if (priceType === 'wholesale') {
    itemPrice = parseFloat(selectedOption.getAttribute('data-wholesale'));
} else {
    itemPrice = parseFloat(selectedOption.getAttribute('data-retail'));
}
        ...
        if (quantity > availableStock) {
            showAlert(`Insufficient stock! Only ${formatNumber(availableStock)} Kg available for ${itemName}`);
            return;
        }
        ...
        if (existingItemIndex !== -1) {
            ...
            if (newTotalQty > availableStock) {
                showAlert(`Cannot add more. Total quantity (${formatNumber(newTotalQty)} Kg) would exceed available stock (${formatNumber(availableStock)} Kg)`);
                return;
            }
...
selectedItemPrice.textContent = formatMoney(price);
    ...
    // Also allow Enter key on select item
