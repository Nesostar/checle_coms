                    ...
                    <tbody>
                        @foreach($categories as $category)
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-warning btn-sm"onclick="openCategoryEditModal({{ $category->id }}, '{{ $category->name }}')"><i class="fa fa-edit"></i> Edit</button>
                                    <form action="{{ route('cashier.items.categories.destroy', $category->id) }}" method="POST" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
...
<div class="modal fade" id="createCategoryModal" tabindex="-1" aria-labelledby="createCategoryLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form id="createCategoryForm" method="POST" action="{{ route('cashier.items.categories.store') }}">
                @csrf
...
$(document).ready(function() {
    ...
    $('#createCategoryForm').on('submit', function(e) {
        ...
        $.ajax({
            url: "{{ route('cashier.items.categories.update', ':id') }}",
            method: "POST",
            data: $(this).serialize(),
...
function openCategoryEditModal(id, name) {
    ...
    let url = "{{ route('cashier.items.categories.update', ':id') }}";
